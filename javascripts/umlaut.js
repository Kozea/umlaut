...
var Actor, Aggregation, Arrow, Association, Base, Battery, BlackArrow, Case, Class, ClassDiagram, Comment, Composition, Container, Database, Decision, Delay, Diagram, Diamond, Diode, Document, Electric, ElectricDiagram, Element, Ellipsis, Flow, FlowChart, Group, HardDisk, IO, Inheritance, InternalStorage, Link, Lozenge, ManualInput, Marker, Mouse, Node, Note, Preparation, Process, Rect, Resistor, SubProcess, Svg, System, Terminator, UseCase, Void, WhiteArrow, WhiteDiamond, Wire, commands, dist, edit, enter_link, enter_node, force_drag, generate_url, ghost_drag, history_pop, init_commands, link_add, list_diagrams, load, mouse_link, mouse_node, mouse_xy, node_add, save, tick_link, tick_node, uml_elements, uml_links, update_link, update_node, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15, _ref16, _ref17, _ref18, _ref19, _ref2, _ref20, _ref21, _ref22, _ref23, _ref24, _ref25, _ref26, _ref27, _ref28, _ref29, _ref3, _ref30, _ref31, _ref32, _ref33, _ref34, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
...
Node = (function(_super) {
  ...
  Node.prototype.direction = function(x, y) {
    return this["super"]('direction', Electric, [x, y]);
  };
...
Battery = (function(_super) {
  ...
  function Battery() {
    _ref32 = Battery.__super__.constructor.apply(this, arguments);
    return _ref32;
  }
...
Wire = (function(_super) {
  ...
  function Wire() {
    _ref33 = Wire.__super__.constructor.apply(this, arguments);
    return _ref33;
  }

  Wire.prototype.path = function() {
    var c1, c2, path, _ref34;
    c1 = this.source.pos();
    c2 = this.target.pos();
    if (void 0 === c1.x || void 0 === c1.y || void 0 === c2.x || void 0 === c2.y) {
      return 'M 0 0';
...
Class = (function(_super) {
  ...
  function Class() {
    _ref34 = Class.__super__.constructor.apply(this, arguments);
    return _ref34;
  }
...
commands = {
  ...
  edit: {
    fun: function() {
      edit((function() {
        ...
        var node, _i, _len, _ref35, _results;
        _ref35 = diagram.selection;
        _results = [];
        for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
          node = _ref35[_i];
  ...
  remove: {
    fun: function() {
      var lnk, node, _i, _j, _len, _len1, _ref35, _ref36;
      _ref35 = diagram.selection;
      for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
        node = _ref35[_i];
  ...
  "switch": {
    fun: function() {
      var link, node, _i, _j, _len, _len1, _ref35, _ref36, _ref37, _ref38;
      _ref35 = diagram.selection;
      for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
        node = _ref35[_i];
        if (node instanceof Link) {
          _ref36 = [node.target, node.source], node.source = _ref36[0], node.target = _ref36[1];
        }
        if (node instanceof Element) {
          _ref37 = diagram.links;
          for (_j = 0, _len1 = _ref37.length; _j < _len1; _j++) {
            link = _ref37[_j];
            _ref38 = [link.target, link.source], link.source = _ref38[0], link.target = _ref38[1];
          }
        }
      }
      return svg.tick();
...
init_commands = function() {
  var e, e1, e2, element, fun, hotkey, i, icon, key, l, link, margin, svgicon, taken_hotkeys, _i, _j, _len, _len1, _ref35, _ref36, _results;
  taken_hotkeys = [];
  ...
  _ref35 = diagram.types.elements.concat(diagram.types.groups);
  for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
    e = _ref35[_i];
    i = 1;
    ...
    element.call(update_node).call(update_node);
    margin = 3;
...
ghost_drag = d3.behavior.drag().on("dragstart", function(node) {
  if (node.cls.resizeable && d3.event.sourceEvent.which === 1) {
    ghost_drag.resize = true;
    d3.event.sourceEvent.stopPropagation();
  }
  if (node.cls.rotationable && d3.event.sourceEvent.which === 2) {
    ghost_drag.rotate = true;
    return d3.event.sourceEvent.stopPropagation();
  }
}).on("drag", function(node) {
  var direction, group, mouse, rotations;
  ...
  if (!ghost_drag.resize && !ghost_drag.rotate) {
    return;
  }
...
mouse_node = function(node) {
  ...
  return node.on("mousedown", function(node) {
    ...
    var lnk, _i, _len, _ref35, _results;
    if (d3.event.ctrlKey) {
      return;
    }
    _ref35 = diagram.linking;
    _results = [];
    for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
      lnk = _ref35[_i];
...
enter_node = function(node) {
  var g;
  g = node.append('g').attr('class', function(node) {
...
update_node = function(node) {
  node.select('text').each(function(node) {
    var i, line, tspan, txt, _i, _len, _ref35, _results;
    txt = d3.select(this);
    if (node.text === txt.text) {
      return;
    }
    txt.selectAll('tspan').remove();
    _ref35 = node.text.split('\n');
    _results = [];
    for (i = _i = 0, _len = _ref35.length; _i < _len; i = ++_i) {
      line = _ref35[i];
  ...
  return node.each(function(node) {
    return $(this).find('path').attr('d', node.path());
  });
};

enter_link = function(link) {
  var g;
  g = link.append('g').attr("class", "link");
  g.append("path").attr('class', 'ghost');
  g.append("path").attr('class', function(lnk) {
    return "shape " + lnk.cls.type;
  }).attr("marker-end", function(lnk) {
    return "url(#" + lnk.cls.marker.id + ")";
  });
...
update_link = function(link) {
  ...
  return link.select('text.end').each(function(lnk) {
    var i, line, tspan, txt, _i, _len, _ref35, _results;
    txt = d3.select(this);
...
tick_node = function(node) {
  return node.attr("transform", (function(node) {
...
tick_link = function(link) {
  link.classed('selected', function(lnk) {
...
Svg = (function(_super) {
  ...
  function Svg() {
    ...
    this.zoom = d3.behavior.zoom().scale(diagram.zoom.scale).translate(diagram.zoom.translate).scaleExtent([.15, 5]).on("zoom", function() {
      var _ref35;
      if (!d3.event.sourceEvent || ((_ref35 = d3.event.sourceEvent.type) === 'wheel' || _ref35 === 'click') || d3.event.sourceEvent.ctrlKey || d3.event.sourceEvent.which === 2) {
    ...
    this.svg.on("mousedown", function(event) {
      var elt, mouse, type, _i, _len, _ref35;
  ...
  Svg.prototype.tick = function() {
    var elt, need_force, _i, _len, _ref35;
    need_force = false;
    _ref35 = diagram.nodes();
    for (_i = 0, _len = _ref35.length; _i < _len; _i++) {
      elt = _ref35[_i];
...
list_diagrams = function() {
  var $tbody, $tr, $ul, b64_diagram, diagram, key, name, title, type, _ref35, _ref36, _results;
  $tbody = $('#diagrams tbody');
  $tbody.find('.local').remove();
  for (key in localStorage) {
    b64_diagram = localStorage[key];
    _ref35 = key.split('|'), type = _ref35[0], title = _ref35[1];
    if (title == null) {
      continue;
    }
    $tbody.append($tr = $('<tr>'));
