...
(function() {
  var E, Element, L, Link, Mouse, article, aside, command, commands, data, defs, drag, e, edit, element, element_add, force, generate_url, height, history_pop, i, key, link, load, mouse_xy, name, objectify, pattern, root, save, svg, sync, taken_hotkeys, tick, underlay, underlay_g, width, zoom, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  Link = (function() {
    function Link(source, target, text_source, text_target) {
      this.source = source;
      this.target = target;
    ...
    Link.prototype.path = function() {
      var c1, c2, d1, d2, horz, m, mid, path, vert;
      c1 = this.source.pos();
      c2 = this.target.pos();
      ...
      this.a1 = this.source.anchor(d1);
      this.a2 = this.target.anchor(d2);
  ...
  sync = function() {
    ...
    link.select('text.start').each(function(lnk) {
      var i, line, tspan, txt, _i, _len, _ref5, _results;
      if (!lnk.text_source) {
        return;
      }
      txt = d3.select(this);
      txt.selectAll('tspan').remove();
      _ref5 = lnk.text_source.split('\n');
      _results = [];
      for (i = _i = 0, _len = _ref5.length; _i < _len; i = ++_i) {
        line = _ref5[i];
        tspan = txt.append('tspan').text(line);
        if (i !== 0) {
      ...
      return lnk._txt_bbox = this.getBBox();
    }).attr('x', function(lnk) {
      return lnk.a1.x;
    }).attr('y', function(lnk) {
      return lnk.a1.y;
  ...
  tick = function() {
    ...
    return link.select('path').attr("d", function(lnk) {
      return lnk.path();
    });
  };
  ...
  element_add = function(type) {
    var elt, new_elt, x, y, _i, _len, _ref5;
    if (state.freemode) {
      x = y = void 0;
    } else {
      x = state.mouse.x;
    ...
    new_elt = new E[type](x, y, 'New element', !state.freemode);
