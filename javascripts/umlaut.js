...
var Arrow, DataIO, Decision, Delay, Diagram, Element, FlowChart, Link, Mouse, Process, State, Svg, Terminator, commands, dist, edit, element_add, generate_url, history_pop, init_commands, load, mouse_xy, save, state, _ref, _ref1, _ref2, _ref3, _ref4, _ref5,
...
init_commands = function() {
  ...
  for (name in commands) {
    ...
    if (!command.fun) {
      _results.push(aside.append('h3').text(command.label));
    } else {
...
Svg = (function() {
  ...
  Svg.prototype.sync = function() {
    ...
    element_g = this.element.enter().append('g').attr('class', 'element').call(this.drag).on("mousedown", function(elt) {
      var selected;
      if (d3.event.ctrlKey) {
        return selected = d3.select(this).classed('selected');
      }
...
$(function() {
  var $diagrams, $editor, $tbody, $tr, b64_diagram, key, title, type, _ref6;
  $diagrams = $('#diagrams');
  $editor = $('#editor');
  ...
  for (key in localStorage) {
    ...
    $tbody.append($tr = $('<tr>'));
    $tr.append($('<td>').text(title), $('<td>').text(_this[type].label), $('<td>').text($('<a>').attr('href', "#" + b64_diagram)));
  }
