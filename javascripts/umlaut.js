...
Element = (function(_super) {
  ...
  Element.prototype.direction = function(x, y) {
    var anchor, angle, diff, min_anchor, pos, _ref;
    angle = atan2(y - this.y, x - this.x);
    diff = Infinity;
    _ref = this.anchors;
    for (anchor in _ref) {
      pos = _ref[anchor];
      if (Math.abs(anchor + this._rotation - angle) < diff) {
        min_anchor = anchor;
        diff = Math.abs(anchor + this._rotation - angle);
      }
    }
    return min_anchor;
  };
...
Link = (function(_super) {
  ...
  Link.prototype.path = function() {
    ...
    d1 = +((this.source_anchor != null) && this.source_anchor !== null ? this.source_anchor : this.source.direction(c2.x, c2.y));
    this.a1 = this.source.rotate(this.source.anchors[d1]());
    d2 = +((this.target_anchor != null) && this.target_anchor !== null ? this.target_anchor : this.target.direction(this.a1.x, this.a1.y));
    ...
    if (diagram.linkstyle === 'demicurve') {
      ...
      dx = Math.cos(d1 + this.source._rotation) * d;
      dy = Math.sin(d1 + this.source._rotation) * d;
      path = "" + path + " " + (this.a1.x + dx) + " " + (this.a1.y + dy);
      dx = Math.cos(d2 + this.target._rotation) * d;
      dy = Math.sin(d2 + this.target._rotation) * d;
...
Wire = (function(_super) {
  ...
  Wire.prototype.path = function() {
    ...
    if ((_ref38 = angle_to_cardinal(this.source._rotation)) === 'E' || _ref38 === 'W') {
      path = "" + path + " " + this.a2.x + " " + this.a1.y + " L";
    } else {
      path = "" + path + " " + this.a1.x + " " + this.a2.y + " L";
    }
...
commands = {
  ...
  freemode: {
    fun: function() {
      ...
      diagram.force = d3.layout.force().gravity(.7).linkDistance(200).charge(function(node) {
...
move_drag = d3.behavior.drag().on('dragstart', function(node) {
  ...
  var delta, x, y, _i, _j, _len, _len1, _ref39, _ref40, _ref41;
  ...
  var lnk, _i, _j, _len, _len1, _ref39, _ref40;
...
mouse_anchor = function(anchor) {
  ...
  return anchor.on("mousemove", function(anchor) {
    ...
    var lnk, node, _i, _len, _ref39, _ref40;
    if (d3.event.ctrlKey) {
      return;
    }
    ...
    if (diagram.linking.length) {
      ...
      for (_i = 0, _len = _ref39.length; _i < _len; _i++) {
        lnk = _ref39[_i];
        if (lnk.source !== lnk.target && ((_ref40 = diagram.mouse) !== lnk.source && _ref40 !== lnk.target)) {
