...
var Actor, Arrow, Association, Case, DataIO, Decision, Delay, Diagram, Element, FlowChart, Inheritance, Link, Mouse, Process, Svg, Terminator, UseCase, commands, dist, edit, element_add, generate_url, history_pop, init_commands, link_add, list_diagrams, load, mouse_xy, save, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
...
Delay = (function(_super) {
  ...
  Delay.prototype.shift = 10;

  Delay.prototype.path = function() {
    var h2, w2;
    w2 = this.width() / 2;
    h2 = this.height() / 2;
    return "M " + (-w2) + " " + (-h2) + "         L " + (w2 - this.shift) + " " + (-h2) + "         Q " + w2 + " " + (-h2) + " " + w2 + " " + (-h2 + this.shift) + "         L " + w2 + " " + (h2 - this.shift) + "         Q " + w2 + " " + h2 + " " + (w2 - this.shift) + " " + h2 + "         L " + (-w2) + " " + h2 + "         z";
  };
...
Association = (function(_super) {
  ...
  Association.prototype.marker = 'arrow';
...
Inheritance = (function(_super) {
  ...
  function Inheritance() {
    _ref9 = Inheritance.__super__.constructor.apply(this, arguments);
    return _ref9;
  }

  Inheritance.prototype.marker = 'lozenge';
...
commands = {
    ...
    fun: function() {
      edit((function() {
        if (diagram.selection.length === 1) {
          return diagram.selection[0].txt;
        } else {
          return '';
        }
      }), (function(txt) {
        var elt, _i, _len, _ref10, _results;
        _ref10 = diagram.selection;
        _results = [];
        for (_i = 0, _len = _ref10.length; _i < _len; _i++) {
          elt = _ref10[_i];
  ...
  remove: {
    fun: function() {
      var elt, lnk, _i, _j, _len, _len1, _ref10, _ref11;
      _ref10 = diagram.selection;
      for (_i = 0, _len = _ref10.length; _i < _len; _i++) {
        elt = _ref10[_i];
        diagram.elements.splice(diagram.elements.indexOf(elt), 1);
        _ref11 = diagram.links.slice();
...
init_commands = function() {
  var e, e1, e2, fun, g, hotkey, i, icon, key, l, margin, path, svgicon, taken_hotkeys, txt, _i, _j, _len, _len1, _ref10, _ref11, _results;
  taken_hotkeys = [];
  ...
  for (_j = 0, _len1 = _ref11.length; _j < _len1; _j++) {
    ...
    path = svgicon.append('path').attr("class", "link").attr("marker-end", "url(#" + icon.marker + ")").attr('d', icon.path());
    margin = 3;
...
Svg = (function() {
  function Svg() {
    ...
    this.drag = this.force.drag().on("drag.force", function(elt) {
      var delta, _i, _len, _ref10, _ref11;
      if (!diagram.dragging || d3.event.sourceEvent.ctrlKey) {
        return;
      }
      if (_ref10 = !elt, __indexOf.call(diagram.selection, _ref10) >= 0) {
    ...
    this.zoom = d3.behavior.zoom().scale(diagram.zoom.scale).translate(diagram.zoom.translate).scaleExtent([.15, 5]).on("zoom", function() {
      var _ref10;
      if (!d3.event.sourceEvent || ((_ref10 = d3.event.sourceEvent.type) === 'wheel' || _ref10 === 'click') || d3.event.sourceEvent.ctrlKey || d3.event.sourceEvent.which === 2) {
  ...
  Svg.prototype.sync = function() {
    ...
    element_g = this.element.enter().append('g').attr('class', 'element').call(this.drag).on("mousedown", function(elt) {
      ...
      var lnk, _i, _len, _ref10, _results;
      if (d3.event.ctrlKey) {
        return;
      }
      _ref10 = diagram.linking;
      _results = [];
      for (_i = 0, _len = _ref10.length; _i < _len; _i++) {
        lnk = _ref10[_i];
    ...
    this.element.select('text').each(function(elt) {
      var i, line, tspan, txt, _i, _len, _ref10, _results;
      txt = d3.select(this);
      if (elt.text === txt.text) {
        return;
      }
      txt.selectAll('tspan').remove();
      _ref10 = elt.text.split('\n');
      _results = [];
  ...
  Svg.prototype.tick = function() {
    var elt, need_force, _i, _len, _ref10;
    need_force = false;
    _ref10 = diagram.elements;
    for (_i = 0, _len = _ref10.length; _i < _len; _i++) {
      elt = _ref10[_i];
...
list_diagrams = function() {
  var $tbody, $tr, $ul, b64_diagram, diagram, key, name, title, type, _ref10, _ref11, _results;
  $tbody = $('#diagrams tbody');
  $tbody.find('.local').remove();
  for (key in localStorage) {
    b64_diagram = localStorage[key];
    _ref10 = key.split('|'), type = _ref10[0], title = _ref10[1];
    if (title == null) {
      continue;
    }
    $tbody.append($tr = $('<tr>'));
