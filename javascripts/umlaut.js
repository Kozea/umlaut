...
Actor = (function(_super) {
  ...
  Actor.prototype.path = function() {
    ...
    bottom = this.height() / 2 - Actor.__super__.txt_height.apply(this) + this.margin.y;
...
Class = (function(_super) {
  ...
  function Class() {
    _ref29 = Class.__super__.constructor.apply(this, arguments);
    return _ref29;
  }
...
link_add = function(type) {
  var elt, _i, _len, _ref30;
  diagram.linking = [];
  _ref30 = diagram.selection;
...
commands = {
  ...
  edit: {
    fun: function() {
      edit((function() {
        ...
        var elt, _i, _len, _ref30, _results;
        _ref30 = diagram.selection;
        _results = [];
        for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
          elt = _ref30[_i];
  ...
  remove: {
    fun: function() {
      var elt, lnk, _i, _j, _len, _len1, _ref30, _ref31;
      _ref30 = diagram.selection;
      for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
        elt = _ref30[_i];
  ...
  "switch": {
    fun: function() {
      var link, node, _i, _j, _len, _len1, _ref30, _ref31, _ref32, _ref33;
      _ref30 = diagram.selection;
      for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
        node = _ref30[_i];
        if (node instanceof Link) {
          _ref31 = [node.target, node.source], node.source = _ref31[0], node.target = _ref31[1];
        }
        if (node instanceof Element) {
          _ref32 = diagram.links;
          for (_j = 0, _len1 = _ref32.length; _j < _len1; _j++) {
            link = _ref32[_j];
            _ref33 = [link.target, link.source], link.source = _ref33[0], link.target = _ref33[1];
          }
        }
      }
      return svg.tick();
...
init_commands = function() {
  var e, e1, e2, fun, g, hotkey, i, icon, key, l, margin, path, svgicon, taken_hotkeys, txt, _i, _j, _len, _len1, _ref30, _ref31, _results;
  taken_hotkeys = [];
  ...
  _ref30 = diagram.types.elements.concat(diagram.types.groups);
  for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
    e = _ref30[_i];
    i = 1;
...
Svg = (function() {
  function Svg() {
    ...
    this.zoom = d3.behavior.zoom().scale(diagram.zoom.scale).translate(diagram.zoom.translate).scaleExtent([.15, 5]).on("zoom", function() {
      var _ref30;
      if (!d3.event.sourceEvent || ((_ref30 = d3.event.sourceEvent.type) === 'wheel' || _ref30 === 'click') || d3.event.sourceEvent.ctrlKey || d3.event.sourceEvent.which === 2) {
  ...
  Svg.prototype.sync = function() {
    ...
    force_drag = this.force.drag().on("drag.force", function(elt) {
      var delta, _i, _len, _ref30, _ref31;
      if (!diagram.dragging || d3.event.sourceEvent.ctrlKey) {
        return;
      }
      if (_ref30 = !elt, __indexOf.call(diagram.selection, _ref30) >= 0) {
    ...
    group_g = group.enter().append('g').attr("class", "group").call(force_drag).on("mousedown", function(grp) {
      ...
      var lnk, _i, _len, _ref30, _results;
      if (d3.event.ctrlKey) {
        return;
      }
      _ref30 = diagram.linking;
      _results = [];
      for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
        lnk = _ref30[_i];
    ...
    group.select('text').each(function(elt) {
      var i, line, tspan, txt, _i, _len, _ref30, _results;
      txt = d3.select(this);
      if (elt.text === txt.text) {
        return;
      }
      txt.selectAll('tspan').remove();
      _ref30 = elt.text.split('\n');
      _results = [];
  ...
  Svg.prototype.tick = function() {
    var elt, link, need_force, _i, _len, _ref30;
    need_force = false;
    _ref30 = diagram.nodes();
    for (_i = 0, _len = _ref30.length; _i < _len; _i++) {
      elt = _ref30[_i];
...
list_diagrams = function() {
  var $tbody, $tr, $ul, b64_diagram, diagram, key, name, title, type, _ref30, _ref31, _results;
  $tbody = $('#diagrams tbody');
  $tbody.find('.local').remove();
  for (key in localStorage) {
    b64_diagram = localStorage[key];
    _ref30 = key.split('|'), type = _ref30[0], title = _ref30[1];
    if (title == null) {
      continue;
    }
    $tbody.append($tr = $('<tr>'));
