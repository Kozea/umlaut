...
(function() {
  var E, L, Mouse, article, aside, command, commands, data, drag, e, element, element_add, force, generate_url, height, history_pop, i, key, link, load, mouse_xy, name, objectify, root, save, svg, sync, taken_hotkeys, tick, underlay, underlay_g, width, zoom, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  underlay = underlay_g.append('rect').attr('class', 'underlay').attr('width', width).attr('height', height);
  ...
  zoom = d3.behavior.zoom().scale(1).on("zoom", function() {
    ...
    if (!d3.event.sourceEvent || ((_ref5 = d3.event.sourceEvent.type) === 'wheel' || _ref5 === 'click') || d3.event.sourceEvent.ctrlKey) {
      ...
      return root.attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")");
    } else {
