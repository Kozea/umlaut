...
Element = (function(_super) {
  ...
  Element.prototype.direction = function(x, y) {
    return angle_to_cardinal(atan2(y - this.y, x - this.x));
  };
...
Link = (function(_super) {
  ...
  Link.prototype.path = function() {
    ...
    this.d2 = this.target_anchor || this.target.direction(this.a1.x, this.a1.y);
    this.a2 = this.target.rotate(this.target.anchors[this.d2]());
    this.d1 = this.source_anchor || this.source.direction(this.a2.x, this.a2.y);
    this.a1 = this.source.rotate(this.source.anchors[this.d1]());
    this.d2 = this.target_anchor || this.target.direction(this.a1.x, this.a1.y);
    this.a2 = this.target.rotate(this.target.anchors[this.d2]());
...
nsweo_resize_drag = d3.behavior.drag().on("dragstart", function(handle) {
  ...
  var angle, delta, m, node, nodes, shift, signs;
  if (d3.event.ctrlKey) {
    return;
  }
...
tick_node = function(nodes) {
  return nodes.attr("transform", (function(node) {
    ...
    return d3.select(this).selectAll('.anchor').data(node.anchor_list()).attr('d', function(anchor) {
      ...
      return "M " + (a.x + signs.x * s) + " " + (a.y + s + signs.y * s) + "                     A " + s + " " + s + " 0 1 1 " + (a.x + signs.x * s) + " " + (a.y - s + signs.y * s) + "                     A " + s + " " + s + " 0 1 1 " + (a.x + signs.x * s) + " " + (a.y + s + signs.y * s);
    });
  });
};
